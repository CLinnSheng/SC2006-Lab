...
export interface SortOptions {
  ...
  active: boolean; // New property to track if sorting is active
}
...
const Sort = ({ onApplySort, currentSort }: SortProps) => {
  ...
  const handleOptionSelect = (option: SortMenuOption) => {
    ...
    if (option.id === 'reset') {
      // Reset to default sorting - distance ascending but mark as inactive
      onApplySort({
        sortBy: 'distance', // Default sort option
        direction: 'asc',   // Default direction
        active: false       // Sorting is not active
      });
    } 
  ...
  // Add a reset option to the dropdown, explicitly typing it to match SortMenuOption
  const allOptions: SortMenuOption[] = [
    ...sortByOptions,
    { id: 'reset', label: 'Cancel Sorting', sortBy: 'distance' as SortOption }
  ];
