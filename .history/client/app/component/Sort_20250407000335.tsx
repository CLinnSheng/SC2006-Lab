...
const Sort = ({ onApplySort, currentSort }: SortProps) => {
  ...
  const [isSorting, setIsSorting] = useState(true); // Track if sorting is active

  const handleOptionSelect = (option: SortMenuOption) => {
    // If this is the reset option
    if (option.id === 'reset') {
      // Reset to default sorting - distance ascending
      onApplySort({
        sortBy: 'distance', // Default sort option
        direction: 'asc'    // Default direction
      });
      setIsSorting(false); // Indicate sorting is canceled
    } 
    // If the same option is selected, toggle the direction
      ...
      setIsSorting(true); // Indicate sorting is active
    } else {
